...
st.write("VERSION: ATLAS-MODE v0.3")
...
# ================== UI ==================
st.title("📚 منصة أطلس السنة – التحقيق النصي (MVP)")

tab1, tab2 = st.tabs(["🔎 البحث الأطلسي", "ℹ️ ملاحظات منهجية"])

# ================== TAB 1 ==================
with tab1:
    st.subheader("🔎 البحث عن الحديث وبناء وحدة أطلسية")

    query = st.text_area(
        "نص البحث (يمكن إدخال أكثر من حديث أو جملة)",
        height=120,
    ...
    with col2:
        top_k = st.slider("عدد النتائج (في الوضع العادي)", 5, 200, 30, 5)
    with col3:
    ...
    if st.button("ابحث", type="primary"):
        if not query.strip():
            ...
            for q in queries:
                ...
                temp = df.copy()
                temp["contains"] = temp["matn_norm"].apply(lambda x: q_norm in x)
                temp["fuzzy"] = temp["matn_norm"].apply(lambda x: fuzz.token_set_ratio(q_norm, x))
                ...
                if search_mode == "احتواء النص":
                    ...
                    temp = temp[(temp["contains"]) | (temp["similarity"] >= float(min_sim))]
            ...
            if results.empty:
                ...
                st.success(f"تم العثور على {len(results)} رواية ضمن {results['hadith_key'].nunique()} وحدة حديثية.")
...
with tab2:
    st.info(
        "هذه المنصة تعمل كأداة تحضير أطلسية:\n"
        "- لا تصدر أحكامًا\n"
        "- لا تفصل السند آليًا\n"
        "- تُستخدم لبناء بطاقة الحديث الأطلسية يدويًا/نصف آلي"
    )
